...
df=pd.read_csv('trajectories(table 5)_training.csv',header=0,usecols=['intersection_id','tollgate_id','vehicle_id','starting_time','travel_time'],index_col=['vehicle_id'])
...
numdays = 91     #setting the day loops you want to make in date_list.
numbmin = 7     #seperate one hour into how many parts. 
    ...
    if i%numbmin==0:
        ...
        if run == 1:
            ...
            df.loc[(df.starting_time >= date_list[i-1]) #comparing if the df.starting_time is late to date_list[0](2016/07/19 08:00) 
                ...
                & (df.intersection_id == in_inter) #selecting the row whose intersection_id is B.
                & (df.tollgate_id == int_toll)].to_csv(output_file) #selecting the row whose tollgate_id is 3 and writing it into output.csv 
            
        else:
            df.loc[(df.starting_time >= date_list[i-1]) 
                & (df.starting_time <= date_list[i])
                & (df.intersection_id == in_inter)
                & (df.tollgate_id == int_toll)].to_csv(output_file, mode='a') #writring into output.csv,but just "adding" instead of overwriting.
